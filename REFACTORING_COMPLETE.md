...
**DESPUÉS:**
```
src/app/projects/ethernicapsule/lib/audio.ts  ✨ Específico del proyecto
```
...
- `/ethernicapsule/unlock` → `/projects/ethernicapsule/unlock`
...
### 4. **URLs Semánticas**
- ✅ `/projects/ethernicapsule` (antes `/ethernicapsule`)
- ✅ `/projects/orbit` (consistente)
...
### Archivos Movidos (12)
1. `src/app/ethernicapsule/` → `src/app/projects/ethernicapsule/`
2. `src/app/lib/audio.ts` → `src/app/projects/ethernicapsule/lib/audio.ts`
...
// DESPUÉS
import { audioAPI } from '@/app/projects/ethernicapsule/lib/audio';
```
...
### Mediano Plazo
1. Considerar mover `/transmission-confirmed/` → `/projects/orbit/confirmed/`
2. Considerar mover `/archive/[id]/` → `/projects/orbit/archive/[id]/`
3. Crear `/projects/figurines/` cuando esté listo
