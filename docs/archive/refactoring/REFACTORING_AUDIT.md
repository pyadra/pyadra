...
- URLs inconsistentes (`/ethernicapsule` vs `/exhibitions/galaxy/orbit`)
...
**Recomendación**: **MOVER** a `/exhibitions/galaxy/ethernicapsule/lib/audio.ts`
...
**Recomendación**:
- **MOVER** `/transmission-confirmed/` → `/exhibitions/galaxy/orbit/confirmed/`
- **MOVER** `/archive/[id]/` → `/exhibitions/galaxy/orbit/archive/[id]/`
...
### Breaking Changes
- **URLs cambiarán**: `/ethernicapsule` → `/exhibitions/galaxy/ethernicapsule`
...
export function middleware(request: NextRequest) {
  ...
  if (request.nextUrl.pathname.startsWith('/ethernicapsule')) {
    return NextResponse.redirect(
      new URL(request.nextUrl.pathname.replace('/ethernicapsule', '/exhibitions/galaxy/ethernicapsule'), request.url)
    );
  }
}
```
